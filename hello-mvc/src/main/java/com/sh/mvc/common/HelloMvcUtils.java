...
public class HelloMvcUtils {
    ...
    static String nextDisabled = """
            <li>
                <a href="#" class="flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg cursor-not-allowed dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                    <span class="sr-only">Next</span>
                    <svg class="w-2.5 h-2.5 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
    ...
    public static String getPagebar(int page, int limit, int totalCount, String url) {
        // 계속 더해줄 것이기 때문에 String대신 StringBuilder 사용
        ...
        if(pageNo == 1) {
            ...
            // 활성화 이전
            pagebar.append(previous.formatted(url + (pageNo - 1))); // /admin/memberList?page=5
        }
