...
<%--                        <c:if test="${hobby ne '운동' && hobby ne'게임' && hobby ne'영화' && hobby ne '음악'}">--%>
                        <c:if test="${not hobbyDefaultList.contains(hobby)}">
                            <div class="inline-flex items-center mr-4">
                                <input id="hobby-${vs.count + hobbyDefaultList.size()}" type="checkbox" name="hobby" value="${hobby}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2" checked>
                ...
                <p class="text-sm font-light text-red-500">
                    서비스를 그만 사용하고 싶으신가요? <a href=s"javascript:confirm('정말 탈퇴하겠습니까?😥😥')&&document.memberDeleteFrm.submit();" class="font-medium text-red-600 hover:underline">여기서 회원탈퇴하세요😭</a>
                </p>
            </form>
        </div>
    </div>
